...
def optimize_harness(ips_instance, problem_setup, cost_field_weights, bundle_weight, save_harness=True, id=0):
    new_field = combine_cost_fields(problem_setup.cost_fields, cost_field_weights, normalize_fields = True)
    new_harness = ips_optimize_harness(ips_instance, problem_setup.harness_setup, new_field, bundle_weight=bundle_weight, save_harness=save_harness, id=id)
    costs = []
...
def evaluate_harness(harness, cost_field):
    ...
    for segment in harness.harness_segments:
        for i in range(len(segment.points)-1):
            ...
            start_coord = cost_field.coordinates[start_node[0], start_node[1], start_node[2]]
